...
const server = new Server({ name: "systemd-manager", version: "1.1.0" }, { capabilities: { tools: {} } });
/**
 * TOOL DEFINITIONS
 */
    ...
    try {
        ...
        switch (name) {
            case "get_service_state": {
                let srv = (args?.service).includes('.') ? args?.service : `${args?.service}.service`;
            ...
            case "manage_service": {
                ...
                let job;
                if (action === 'start')
                    job = await manager.StartUnit(srv, 'replace');
                if (action === 'stop')
                    job = await manager.StopUnit(srv, 'replace');
                if (action === 'restart')
                    job = await manager.RestartUnit(srv, 'replace');
                if (action === 'reload')
                    job = await manager.ReloadUnit(srv, 'replace');
